FROM --platform=$TARGETOS/$TARGETARCH rust:1.91-slim-trixie AS build-image
LABEL org.opencontainers.image.description="K8s job operator"
...
WORKDIR /
COPY --from=build-image /build/target/k8s-job-operator /build/LICENSE /

USER nonroot:nonroot

ENTRYPOINT ["/k8s-job-operator"]
